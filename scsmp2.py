...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, verbose=None):
    ...
    for i in range(nocc):
        ...
        emp2 += numpy.einsum('jab,jab', t2i, gi) * (pt + ps)
        emp2 -= numpy.einsum('jab,jba', t2i, gi) * pt
        if with_t2:
            t2[i] = t2i
...
def energy(mp, t2, eris):
    ...
    emp2  = numpy.einsum('ijab,iajb', t2, eris_ovov) * (pt+ps)
    emp2 -= numpy.einsum('ijab,ibja', t2, eris_ovov) * pt
    return emp2.real
...
def make_rdm2(mp, t2=None, eris=None, ao_repr=False):
    ...
    for i in range(nocc):
        ...
        # notation):
        #   E = einsum('pqrs,pqrs', eri, rdm2)
        dovov = t2i.transpose(1,0,2)*(pt+ps) - t2i.transpose(2,0,1)*pt
        dovov *= 2
        if moidx is None:
...
class SCSMP2(lib.StreamObject):
    '''restricted SCS-MP2 with canonical HF and non-canonical HF reference
    ...
    def __init__(self, mf, l_mp2=False, frozen=None, mo_coeff=None, mo_occ=None):
...
scf.hf.RHF.MP2 = lib.class_as_method(SCSMP2)
